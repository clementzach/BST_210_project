...
preds <- numeric(length = nrow(heart))
num_folds <- 5
...
for (i in 1:num_folds){
  ...
  preds[current_obs] <- predict(current_model, 
...
caret::confusionMatrix(as.factor(as.numeric(preds > min_sens_spec)), 
...
```{r}
fit_val <- ggplot(data.frame(preds = preds), aes(x = preds)) + geom_histogram() + ggtitle("Histogram of Fitted Values") + xlab(expression(hat(p))) 
...
#car::influencePlot(non_lasso, col = "red")
...
plot(preds, my_pearson_resid, xlab = "Predicted Probability", ylab = "Pearson Residuals")
```
...
```{r}
hoslem.test(heart$mortality_30, preds, g = 10)
```
