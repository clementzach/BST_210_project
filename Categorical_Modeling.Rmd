...
1. **The outcome `Death Event` $\sim Binomial(1, p)$, with the sample being independent and identically distributed (IID).**\
...
2. **$\mathbb{E}[\textit{Death Event} | X] = P(Death\;Event = 1) = p$ is linear on the logit scale $\left(logit(p) = \frac{p}{1 - p}\right)$.**\
